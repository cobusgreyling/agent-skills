...
The judge model MUST differ from the model being evaluated. Same-model
self-judging is co-conspirator territory and not a fair eval.
"""
...
def extract_section(text: str, heading: str) -> str:
    """Pull a level-3 markdown section by heading."""
    marker = f"### `{heading}`"
...
def judge_call(prompt: str, judge_model: str) -> str:
    """Call the judge model. Extend for your provider."""
    try:
...
def main() -> int:
    ...
    print(f"{'task_id':<14} {'with_skill':<12} {'without_skill':<14}")
    with_passes = 0
    without_passes = 0
    for tid in sorted(with_records):
        ...
        print(f"{tid:<14} {'PASS' if w_pass else 'FAIL':<12} {'PASS' if wo_pass else 'FAIL':<14}")
