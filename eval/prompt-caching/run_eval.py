#!/usr/bin/env python3
"""Run the prompt-caching eval against an agent.
...
agent of your choice. Provided here for the Anthropic SDK; swap in OpenAI,
...
def call_agent(prompt: str, agent: str, skill_loaded: bool) -> str:
    ...
    Extend this function for each agent you care about. The simplest
    integration is via the agent's chat / completions API directly.
    """
    if agent == "anthropic":
        try:
        ...
        # If skill is loaded in the agent harness, the system prompt is set
        # by that harness. For a bare SDK call without a harness, paste the
        # SKILL.md body into the system prompt when skill_loaded is True.
        system = ""
        if skill_loaded:
            system = (EVAL_DIR.parent.parent / "skills" / "prompt-caching" / "SKILL.md").read_text(encoding="utf-8")
